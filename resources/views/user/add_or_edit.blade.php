...
@section('content')
    <div class="flex items-center justify-center p-12 w-full">
        <div class="mx-auto w-full max-w-[550px]">
            <form action="{{ empty($user) ?  route('companies.store') : route('companies.update',[$user->id]) }}" id="is_validate_form" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="w-full px-3">
                        <div class="mb-5">
                            ...
                            <input type="text" name="title" id="title" placeholder="Company Title"
                                ...
                                data-rule-required="true" value="{{ old('title', $user->title ?? '') }}">
                    ...
                    <div class="w-full px-3">
                        <div class="mb-5">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="0" {{ old('status', $user->status ?? '') == 0 ? 'selected' : '' }}>In
                                    Active
                                </option>
                            </select>
                            @error('status')
